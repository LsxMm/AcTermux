...
public final class TerminalView extends View {
        ...
        new GestureAndScaleRecognizer(
            ...
            new GestureAndScaleRecognizer.Listener() {
              ...
              public boolean onDoubleTap(MotionEvent e) {
                // Old behavior: Do not treat is as a single confirmed tap - it may be followed by
                // zoom.
  ...
  /**
   * @param client Listener for all kinds of key events, both hardware and IME (which makes it
   *     different from that available with {@link View#setOnKeyListener(OnKeyListener)}.
   */
  ...
  void sendMouseEventCode(MotionEvent e, int button, boolean pressed) {
    ...
    int y =
        (int) ((e.getY() - mRenderer.mFontLineSpacingAndAscent) / mRenderer.mFontLineSpacing) + 1;
    if (pressed
    ...
    if (mIsSelectingText) {
      ...
      if (ev.isButtonPressed(MotionEvent.BUTTON_SECONDARY)) {
        ...
        switch (ev.getAction()) {
          ...
          case MotionEvent.ACTION_UP:
            sendMouseEventCode(
                ev, TerminalEmulator.MOUSE_LEFT_BUTTON, ev.getAction() == MotionEvent.ACTION_DOWN);
            break;
          case MotionEvent.ACTION_MOVE:
  ...
  /** 每次处理用户按下对终端输出有影响的键时被调用 如果当前屏幕不处于最底部，则自动滚动到最底部 */
  ...
  protected void onDraw(@NonNull Canvas canvas) {
    if (mEmulator == null) {
      ...
      if (mIsSelectingText) {
        ...
        int top =
            (mSelY1 + 1 - mTopRow) * mRenderer.mFontLineSpacing
                + mRenderer.mFontLineSpacingAndAscent;
        mLeftSelectionHandle.setBounds(
            right - gripHandleWidth,
            top,
            right,
            top + mLeftSelectionHandle.getIntrinsicHeight() / 3);
        mLeftSelectionHandle.draw(canvas);

        int left = Math.round((mSelX2 + 1) * mRenderer.mFontWidth) - gripHandleMargin;
        top =
            (mSelY2 + 1 - mTopRow) * mRenderer.mFontLineSpacing
                + mRenderer.mFontLineSpacingAndAscent;
        mRightSelectionHandle.setBounds(
            left,
            top,
            left + gripHandleWidth,
            top + mRightSelectionHandle.getIntrinsicHeight() / 3);
  ...
  public void toggleSelectingText(MotionEvent ev) {
    ...
    if (mIsSelectingText) {
      ...
      if (!" ".equals(screen.getSelectedText(mSelX1, mSelY1, mSelX1, mSelY1))) {
        ...
        while (mSelX1 > 0
            && !screen.getSelectedText(mSelX1 - 1, mSelY1, mSelX1 - 1, mSelY1).isEmpty()) {
          mSelX1--;
        }
        while (mSelX2 < mEmulator.mColumns - 1
            && !screen.getSelectedText(mSelX2 + 1, mSelY1, mSelX2 + 1, mSelY1).isEmpty()) {
          mSelX2++;
        }
      }
      ...
      final ActionMode.Callback callback =
          new ActionMode.Callback() {
            ...
            public boolean onCreateActionMode(ActionMode mode, Menu menu) {
              ...
              menu.add(Menu.NONE, 2, Menu.NONE, "粘贴")
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
              ...
              switch (item.getItemId()) {
                case 1:
                  String selectedText =
                      mEmulator.getSelectedText(mSelX1, mSelY1, mSelX2, mSelY2).trim();
                  Toast.makeText(getContext(), "已复制", Toast.LENGTH_SHORT).show();

                  // 获取剪贴板管理器
                  ClipboardManager clipboardManager =
                      (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
                  // 创建一个ClipData对象，这里使用ClipData.newPlainText()方法创建一个纯文本的ClipData
                  ClipData clipData = ClipData.newPlainText("复制", selectedText);
                  // 将ClipData对象设置到剪贴板管理器中
